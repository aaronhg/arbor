// Phase-4 safety net: the coir × copse coverage JOIN — pure, no engine. Moved here from copse (was
// test/coverage.test.js). Pins the two-tier match (exact + prefab-internal prefix) and the buckets, so the
// verdict a PR sees can't silently change after the move off copse.
...
test('coverageJoin: >1 suffix candidate → ambiguous, never silently guessed', () => {
  ...
  assert.equal(codeOnly.length, 0); // the candidates are in `ambiguous` — they must NOT also leak into codeOnly
});
